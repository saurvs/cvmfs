  ...
  if (fd & kFdChunked) {
    ...
    do {
      ...
      if ((chunk_fd->fd == -1) || (chunk_fd->chunk_idx != chunk_idx)) {
        ...
        cvmfs::FetchJob job(chunk_list->AtPtr(chunk_idx)->content_hash(),
            chunk_list->AtPtr(chunk_idx)->size(), "no path info", compression_alg,
            CacheManager::kTypeRegular);
        if (open_chunks.chunk_reflist.external_data) {
          std::string alt_url = open_chunks.chunk_reflist.path.ToString();
          job.alt_url = &alt_url;
          job.range_offset = chunk_list->AtPtr(chunk_idx)->offset();
